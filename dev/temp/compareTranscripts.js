(async () => {
    ...
    let queue30s = require('./fileNames')
    let { generateCreateTranscriptionPromise, generateGetTranscriptionPromise, compareWithDBResult, removeExistFiles, promiseAllWithTimeout } = require('./helperXF')
    ...
    async function transcribeCallBatch (createTranscriptionPromises, processingAudioURLs) {
      ...
      // method 1: when createTranscriptionPromises will wait until transcription is returned
      ...
      await compareWithDBResult(transcriptionTasks, processingAudioURLs)
      processedFileCount += transcriptionTasks.length
  
    // // method 2: use getTranscriptionPromises to check if transcription is finished
    ...
    //   queue = await removeExistFiles(queue)
      ...
      await transcribeAllCalls(queue30s)
    } catch (error) {
      debug('error', error)
    }
